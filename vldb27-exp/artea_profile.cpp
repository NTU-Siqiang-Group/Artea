...
public:
    ...
    void init() {
        ...
        ARTEA_INFO(fmt::format("Dataset: {} vectors, {} dims",
...
auto profile_to_json(const Profile1NNResult& result) -> nlohmann::json {
    nlohmann::json hops = nlohmann::json::array();
    for (std::size_t i = 0; i < result.per_hop_statistics.size(); ++i) {
        const auto& p = result.per_hop_statistics[i];
        hops.push_back({
            {"hop",     i},
            {"avg_ndc", p.avg_ndc},
            {"adr",     p.adr}
...
int main(int argc, char** argv) {
    argparse::ArgumentParser program("artea_profile");
    ...
    program.add_argument("-o", "--output")
        .default_value(std::string("./vldb27-exp/results/router_profiler_results.json"))
    ...
    ARTEA_INFO("Profiling hierarchical artea (HGRouterProfiler)...");
    hg_router_profiler_t hg_profiler(base_vecs, dist_func);
    hg_profiler.initialize();
    auto hg_t0 = std::chrono::high_resolution_clock::now();
    Profile1NNResult hg_result =
        hg_profiler.profile(query_vecs, compact_hg, gt_vecs, entry_point_vid);
    auto hg_t1 = std::chrono::high_resolution_clock::now();
    const int64_t hg_ms =
        std::chrono::duration_cast<std::chrono::milliseconds>(hg_t1 - hg_t0).count();
    ARTEA_INFO(fmt::format(
        "Hierarchical profile: {} hops, {} queries ({} skipped), {} ms",
        hg_result.per_hop_statistics.size(),
        hg_result.num_queries, hg_result.skipped_queries, hg_ms));

    // ---- Curve 2: L0-only (SLRouterProfiler on compact_hg level 0) ----
    ARTEA_INFO("Profiling L0-only (SLRouterProfiler)...");
    sl_router_profiler_t sl_profiler(base_vecs, dist_func);
    sl_profiler.initialize();
    auto sl_t0 = std::chrono::high_resolution_clock::now();
    Profile1NNResult sl_result = sl_profiler.profile(
        query_vecs, compact_hg, layer_id_t{0}, gt_vecs, entry_point_vid);
    auto sl_t1 = std::chrono::high_resolution_clock::now();
    const int64_t sl_ms =
        std::chrono::duration_cast<std::chrono::milliseconds>(sl_t1 - sl_t0).count();
    ARTEA_INFO(fmt::format(
        "L0-only profile: {} hops, {} queries ({} skipped), {} ms",
        sl_result.per_hop_statistics.size(),
        sl_result.num_queries, sl_result.skipped_queries, sl_ms));
    ...
    out["curves"] = {
        {"hierarchical_artea", profile_to_json(hg_result)},
        {"l0_only",            profile_to_json(sl_result)}
    };

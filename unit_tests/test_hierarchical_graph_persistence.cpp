...
TEST_F(HierarchicalGraphPersistenceTest, SnapshotAndRestore) {
    ...
    logger.info(fmt::format("Graph construction completed in {:.2f} s", duration.count() / 1000.0));

    const layer_num_t num_layers = original_graph.get_num_layers();
    logger.info(fmt::format("Constructed graph with {} layers", num_layers));

    // Snapshot the graph
    logger.info(fmt::format("Snapshotting graph to {}...", test_index_dir));
    ...
    for (layer_id_t layer_id = 0; layer_id < num_layers; ++layer_id) {
        ...
        logger.info(fmt::format("Layer {} verification passed", layer_id));
    }
    ...
    for (layer_id_t layer_id = 1; layer_id < num_layers; ++layer_id) {
        ...
        logger.info(fmt::format("Layer {} inter-layer links verification passed", layer_id));
    }

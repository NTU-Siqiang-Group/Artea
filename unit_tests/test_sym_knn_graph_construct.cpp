...
// symmetric_knn_graph::index_t is metric/dim-dependent now; the average-degree
// helper only touches the metric-independent neighbor arrays. It is templated
// on the concrete graph type (the <Metric, Dim> index is an alias template, so it
// cannot drive template argument deduction directly).
template <typename Graph>
static auto compute_average_degree(const Graph& graph) -> double {
...
struct TestConfig {
    ...
    // layer_config_t is metric/dim-independent; the propagate config is now a
    // <Metric, Dim> alias, so we keep its raw params here and build the object
    // inside the dispatched body.
...
public:
    ...
    void init() {
        ...
        infra_dispatch(dataset_info_, ARTEA_METRIC_LAMBDA(void) {
            ...
            symmetric_knn_graph::index_t<Metric, Dim> graph_index = std::move(
                symmetric_knn_graph::factory_t<Metric, Dim>::construct_graph(
                    base_vecs,
            ...
            g_test_results.avg_degree = compute_average_degree(graph_index);
...
int main(int argc, char** argv) {
    ...
    std::cout << fmt::format("  Routing Loops:          {}", g_config.num_routing_loops) << std::endl;

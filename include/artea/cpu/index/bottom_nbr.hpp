...
struct alignas(8) BottomNeighbor {   // 8 bytes
...
public:
    constexpr BottomNeighbor() : nbr_id_and_status(invalid_vertex_id), distance(0.0) {}

    static constexpr auto make_invalid_nbr() -> BottomNeighbor {
        return BottomNeighbor{ invalid_vertex_id, max_distance };
    }

    BottomNeighbor(const BottomNeighbor&) = default;
    BottomNeighbor& operator=(const BottomNeighbor&) = default;
    BottomNeighbor(BottomNeighbor&&) = default;
    BottomNeighbor& operator=(BottomNeighbor&&) = default;
    ~BottomNeighbor() = default;

    /** @brief Create a Neighbor with given ID and distance (2-parameter constructor for constexpr). */
    constexpr BottomNeighbor(
    ...
    /** @brief For BottomNeighbor, base_vid == level_vid (single-layer graph). */
    __attribute__((always_inline))
    auto get_base_vid() const -> vertex_id_t {
        return get_level_vid();
    }
    ...
    __attribute__((always_inline))
    constexpr bool operator==(const BottomNeighbor& other) const noexcept {
...
};  // struct BottomNeighbor

// TO enable optimizations for POD types
template <typename BaseTraitsT>
inline constexpr bool __descent_neighbor_is_trivially_copyable =
    std::is_trivially_copyable<BottomNeighbor<BaseTraitsT>>::value;

template <typename BaseTraitsT>
inline constexpr bool __descent_neighbor_is_trivially_destructible =
    std::is_trivially_destructible<BottomNeighbor<BaseTraitsT>>::value;

static_assert(std::is_trivially_copyable<BottomNeighbor<BaseTraits<uint32_t, float>>>::value,
            "Neighbor must be trivially copyable to enable vector memcpy optimizations!");
static_assert(std::is_trivially_destructible<BottomNeighbor<BaseTraits<uint32_t, float>>>::value,
            "Neighbor must be trivially destructible!");

/** @brief Comparator for Neighbor (Distance primary, ID secondary). */
template <typename BaseTraitsT>
struct BNbrComparator {
    using bnbr_t = BottomNeighbor<BaseTraitsT>;

    __attribute__((always_inline))
    constexpr bool operator()(const bnbr_t& a, const bnbr_t& b) const noexcept {

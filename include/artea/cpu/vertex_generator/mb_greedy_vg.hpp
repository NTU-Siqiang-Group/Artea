...
public:
    ...
    auto gen_subset(
        ...
        // Main loop: iteratively add the farthest point from each batch
        while (approx_rnet.get_num_vecs() < max_result_size) {
            ...
            // Store the minimum distance from each candidate to approx_rnet
            std::vector<distance_t> candidate_min_distances(current_batch_size);

            // Parallel computation: for each candidate, compute its minimum distance to approx_rnet
            tbb::parallel_for(
                tbb::blocked_range<vec_num_t>(0, current_batch_size),
                [&](const tbb::blocked_range<vec_num_t>& r) {
                    for (vec_num_t local_idx = r.begin(); local_idx != r.end(); ++local_idx) {
                        ...
                        for (vec_num_t ret_idx = 0; ret_idx < approx_rnet.get_num_vecs(); ++ret_idx) {
                            min_distance = std::min(min_distance, _dist_func(candidate_vec, approx_rnet.vectors.get(ret_idx)));
                        }
            ...
            // Find the candidate with the maximum distance to approx_rnet
            const auto max_it = std::max_element(candidate_min_distances.begin(), candidate_min_distances.end());
            const distance_t max_distance = *max_it;
            ...
            // Add the best candidate to approx_rnet
            const vec_num_t best_local_idx = std::distance(candidate_min_distances.begin(), max_it);
            const vec_id_t best_vec_id = batch_start + best_local_idx;
            approx_rnet.vec_ids.push_back(best_vec_id);
            approx_rnet.vectors.append_vec(base_vecs.get(best_vec_id));
        }

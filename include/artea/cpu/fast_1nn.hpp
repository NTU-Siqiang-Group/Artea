...
public:
    ...
    auto compute(
        ...
        const VectorArray<cluster_num_t, vec_ele_t>* centroids,
        const cluster_num_t num_clusters,
        const cluster_id_t* results
    ) -> void {
        ...
        for (vecs_num_t vec_id = 0; vec_id < _vecs->size(); vec_id++) {
            ...
            *      cluster_id_t best_cluster_id = 0;
            *      vec_ele_t best_dist = std::numeric_limits<vec_ele_t>::max();
            *      for (cluster_id_t cluster_id = 0; cluster_id < num_clusters; cluster_id++) {

...
/*
 * @FilePath: /Artea/include/artea/cpu/router/stateful_candidate_entry.hpp
...
struct alignas(8) StatefulCandidateEntry {   // 8 bytes
...
public:
    constexpr StatefulCandidateEntry() : entry_id_and_status(invalid_vertex_id), distance(0.0) {}

    constexpr StatefulCandidateEntry(const StatefulCandidateEntry&) = default;
    constexpr StatefulCandidateEntry& operator=(const StatefulCandidateEntry&) = default;
    StatefulCandidateEntry(StatefulCandidateEntry&&) = default;
    StatefulCandidateEntry& operator=(StatefulCandidateEntry&&) = default;
    ~StatefulCandidateEntry() = default;

    /** @brief Create a new StatefulCandidateEntry with given ID and distance (unexplored by default). */
    constexpr StatefulCandidateEntry(
        const vertex_id_t entry_id,
        const distance_t distance
    ) : entry_id_and_status(entry_id & MASK_ID), distance(distance) {}

    /** @brief Create a new StatefulCandidateEntry with given ID, distance, and explored status. */
    constexpr StatefulCandidateEntry(
        const vertex_id_t entry_id,
        const distance_t distance,
    ...
    static constexpr auto make_invalid_entry() -> StatefulCandidateEntry {
        return StatefulCandidateEntry(invalid_vertex_id, max_distance);
    }

    /** @brief Create a sentinel entry with minimum distance (for max-heap sentinel). */
    static constexpr auto make_min_entry() -> StatefulCandidateEntry {
        return StatefulCandidateEntry(invalid_vertex_id, min_distance);
    }
    ...
    static auto make_entry(
        const vertex_id_t entry_id,
        const distance_t distance
    ) -> StatefulCandidateEntry {
        return StatefulCandidateEntry{entry_id, distance};
    }
    ...
    __attribute__((always_inline))
    constexpr bool operator<(const StatefulCandidateEntry& other) const noexcept {
...
};  // struct StatefulCandidateEntry

// TO enable optimizations for POD types
template <typename RouterTraitsT>
inline constexpr bool __stateful_candidate_entry_is_trivially_copyable =
    std::is_trivially_copyable<StatefulCandidateEntry<RouterTraitsT>>::value;

template <typename RouterTraitsT>
inline constexpr bool __stateful_candidate_entry_is_trivially_destructible =
    std::is_trivially_destructible<StatefulCandidateEntry<RouterTraitsT>>::value;

/** @brief Comparator for StatefulCandidateEntry by distance. */
template <typename RouterTraitsT>
struct StatefulCandidateEntryComparator {
    using entry_t = StatefulCandidateEntry<RouterTraitsT>;
